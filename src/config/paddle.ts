...
export function openPaddleCheckout(
  ...
  // Logged-in: pass userId so webhook resolves account directly.
  // Guest: pass userId: null so webhook falls back to data.customer.email
  // (Paddle's own checkout form collects the email natively).
  window.Paddle.Checkout.open({
    items: itemsList,
    ...(userEmail ? { customer: { email: userEmail } } : {}),
    customData: userId ? { userId } : { userId: null },
  });
}
